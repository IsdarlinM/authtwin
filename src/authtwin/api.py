...
def create_app(workspace: Path) -> FastAPI:
    app=FastAPI(title="AuthTwin Local API",version=__version__,docs_url="/docs",redoc_url=None);engine=AuthTwinEngine(workspace);shared_graph=TemporalGraph(workspace);shared_jobs=JobEngine(workspace);shared_lineage=EvidenceLineage(workspace);shared_notebook=ResearchNotebook(workspace)
    @app.middleware("http")
    async def headers(request:Any,call_next:Any)->Any:
        response=await call_next(request);response.headers["Content-Security-Policy"]="default-src 'self'; script-src 'self'; style-src 'unsafe-inline'; frame-ancestors 'none'";response.headers["X-Content-Type-Options"]="nosniff";response.headers["Referrer-Policy"]="no-referrer";return response
    ...
    async def summary()->dict[str,object]:
        data=engine.store.load();coverage=engine.coverage();return {"version":__version__,"actors":len(data["actors"]),"resources":len(data["resources"]),"observations":len(data["observations"]),"findings":len(data["findings"]),"coverage":coverage.coverage}
    @app.get("/api/state-machine")
    async def state_machine()->dict[str,list[dict[str,Any]]]:return engine.state_machine()
    @app.get("/api/matrix")
    async def matrix()->dict[str,Any]:return engine.matrix()
    @app.get("/api/coverage")
    async def coverage()->dict[str,Any]:return engine.coverage().model_dump(mode="json")
    @app.get("/api/counterfactuals")
    async def counterfactuals()->list[dict[str,Any]]:return [x.model_dump(mode="json") for x in engine.counterfactuals()]
    @app.get("/api/graphql")
    async def graphql_surface()->list[dict[str,Any]]:return engine.graphql_surface()
    @app.get("/api/batch")
    async def batch_surface()->list[dict[str,Any]]:return engine.batch_authorization()
    @app.get("/api/discovery-v2")
    async def discovery_v2()->dict[str,Any]:return AuthorizationIntelligence(engine).discovery_v2()
    @app.get("/api/bindings")
    async def bindings()->list[dict[str,Any]]:return [x.model_dump(mode="json") for x in AuthorizationIntelligence(engine).infer_actor_resource_bindings()]
    @app.get("/api/state-v2")
    async def state_v2()->dict[str,Any]:return AuthorizationIntelligence(engine).state_machine_v2()
    @app.get("/api/mutation-plans")
    async def mutation_plans()->list[dict[str,Any]]:return [x.model_dump(mode="json") for x in AuthorizationIntelligence(engine).mutation_plans()]
    @app.get("/api/websocket")
    async def websocket_surface_v2()->list[dict[str,Any]]:return AuthorizationIntelligence(engine).websocket_surface()
    @app.get("/api/search")
    async def shared_search(q:str,limit:int=50)->list[dict[str,Any]]:return shared_graph.search(q,max(1,min(limit,500)))
    @app.get("/api/jobs")
    async def shared_jobs_endpoint()->list[dict[str,Any]]:return [x.model_dump(mode="json") for x in shared_jobs.list()]
    ...
    async def shared_notebook_endpoint()->list[dict[str,Any]]:return [x.model_dump(mode="json") for x in shared_notebook.list()]
