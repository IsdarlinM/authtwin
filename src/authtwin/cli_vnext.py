...
def doctor_vnext(
    ...
    """Check Python, exact SRIC feature compatibility, plugins and privacy settings."""
...
def discover_v2(workspace: str, root: Path = typer.Option(root_default(), "--root")) -> None:
...
def state_v2(workspace: str, root: Path = typer.Option(root_default(), "--root")) -> None:
    """Show actor, resource and membership lifecycle state."""
    engine = AuthTwinEngine(ws_path(workspace, root))
    typer.echo(json.dumps(AuthorizationIntelligence(engine).state_machine_v2(), indent=2, default=str))
...
def membership_event(
    ...
    event = MembershipEvent(event_id=event_id, actor_id=actor_id, tenant_id=tenant_id, state=state.upper(), evidence_ids=evidence)
    engine = AuthTwinEngine(ws_path(workspace, root))
...
def differential(
    ...
    engine = AuthTwinEngine(ws_path(workspace, root))
    insight = AuthorizationIntelligence(engine).differential_response(observation_a, observation_b)
    typer.echo(insight.model_dump_json(indent=2))


@app.command("invariant-library")
def invariant_library(workspace: str, root: Path = typer.Option(root_default(), "--root")) -> None:
...
def websocket_surface(workspace: str, root: Path = typer.Option(root_default(), "--root")) -> None:
    """Show evidence-bearing WebSocket authorization lifecycle observations."""
    engine = AuthTwinEngine(ws_path(workspace, root))
    typer.echo(json.dumps(AuthorizationIntelligence(engine).websocket_surface(), indent=2, default=str))
...
def policy_import(
    ...
    report = normalize_policy_export(provider=provider, source_id=source_id, data=raw, evidence_ids=evidence)
