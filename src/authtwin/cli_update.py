...
def update(
    ...
    """Check/install AuthTwin and keep its shared SRIC runtime compatible."""
    if check and force:
        typer.echo("--check and --force cannot be used together.", err=True)
        raise typer.Exit(2)
    custom_requested = bool(manifest or public_key or os.getenv("AUTHTWIN_RELEASE_MANIFEST_URL") or os.getenv("AUTHTWIN_RELEASE_PUBLIC_KEY"))
    try:
        ...
        status = perform_product_update(
            ...
            manifest_env="AUTHTWIN_RELEASE_MANIFEST_URL",
            public_key_env="AUTHTWIN_RELEASE_PUBLIC_KEY",
        )
