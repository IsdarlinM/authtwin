...
rem Resolve AuthTwin and the explicit SRIC source atomically because sric-core is
rem intentionally distributed from an immutable first-party source, not PyPI.
if defined SRIC_CORE_SOURCE (
  if not exist "%SRIC_CORE_SOURCE%\pyproject.toml" (echo SRIC_CORE_SOURCE is invalid.& exit /b 3)
  "%VENV%\Scripts\python.exe" -m pip install --upgrade --force-reinstall -c "%CONSTRAINTS%" "%SRIC_CORE_SOURCE%" "%REPO_ROOT%" || (echo Atomic AuthTwin/SRIC installation failed.& exit /b 3)
) else (
  if not exist "%FIRST_PARTY%" (echo Missing first-party dependency manifest: %FIRST_PARTY%& exit /b 3)
  "%VENV%\Scripts\python.exe" -m pip install --upgrade --force-reinstall -c "%CONSTRAINTS%" -r "%FIRST_PARTY%" "%REPO_ROOT%" || (echo Atomic AuthTwin/SRIC installation failed.& exit /b 3)
)
