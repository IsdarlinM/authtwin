...
def _cell(
    cell_id: str, resource_id: str, *, equivalence_class: str | None = None
) -> UnknownAuthorizationCell:
...
def test_explicitly_equivalent_unknown_cells_are_compressed() -> None:
    plans = build_validation_plan(
        [
            _cell("c1", "r1", equivalence_class="documents"),
            _cell("c2", "r2", equivalence_class="documents"),
        ]
    )
    assert len(plans) == 1
...
def test_different_resources_are_not_implicitly_equivalent() -> None:
