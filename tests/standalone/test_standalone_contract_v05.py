...
OTHER_PRODUCTS = {"reprosec", "fossilscope", "trustboundary", "exposuredna"}
runner = CliRunner()
...
def test_runtime_dependencies_do_not_require_sibling_products() -> None:
    ...
    assert any(item.lower().replace(" ", "").startswith("reprosec") for item in project["optional-dependencies"]["rcap"])
...
def test_every_registered_command_path_supports_all_help_forms() -> None:
    ...
    for path in _command_paths(command):
        ...
        argv = normalize_help_argv(["authtwin", *path, "help"])
